...
class YibanSign:
    ...
    def __init__(self):
        self.ct = Captcha()
        self.label = 'yb_yiban_sign'
        self.env = Env(self.label)
        self.st = Setting(self.label)
        self.client_id = "7af698a43be206c0"

    # 打卡
    def set_sign(self, cookie, account):
        """
        打卡
        :return:
        """
        try:
            ...
            if self.client_id in r1.text:
                oauth_header = {
                    'Host': 'oauth.yiban.cn',
                    'X-Requested-With': 'XMLHttpRequest',
                    'User-Agent': self.env.UserAgent,
                    'Referer': 'https://oauth.yiban.cn/code/html?client_id=' + self.client_id + '&redirect_uri=http://f.yiban.cn/iapp642231',
                }
            ...
            for count in range(3):
                ...
                capt = self.ct.base64_api(base64_image)
                ...
                if resp['code'] == 1:
                    self.ct.report_error(capt['id'])

                    self.st.msg_(resp['code'], '验证码错误', phone=account)
                elif resp['code'] == 2:
    ...
    # 脚本开始
    def run(self):
        if self.ct.ct_user is None or self.ct.ct_pass is None:
            return 0
        result = self.env.get_env('YB_COOKIE')
        if result['code'] != 1:
            self.st.msg_(-999, '易伴打卡: ' + result['msg'])
            return 0
        ...
        for count in range(2):
            for k, i in enumerate(lst):
                ...
                account = lit[0]
                try:
                    result = self.set_sign(i['value'], account)
                    self.st.msg_(result['code'], result['msg'], phone=account)
                    if result['code'] != -1:
                        lst.pop(k)
                except Exception as ex:
                    self.st.msg_(-1, '打卡 %s' % ex, phone=account)
