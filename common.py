...
class Env:
    ...
    def __init__(self, label=None):
        # 0 = sqlite数据库获取, 1 = 外部获取, 2 = 读取excel
        self.mode = 2
        self.label = label
    ...
    def get_excel(self, name):
        try:
            if self.check_excel_file() is False:
                return {'code': -1, 'msg': 'Excel文件不存在'}
            if name != 'YB_COOKIE':
                return {'code': -1, 'msg': f'[{name}]不存在'}
            return Excel(self.check_excel_file()).get_data()
        except Exception as ex:
...
class Setting:
    ...
    def msg_(self, code, msg, data=None, phone=None, label=None):
        ...
        # m = f'[{label}] \t code: {code}\tmsg: {msg}\t{f"user: {phone}" if phone else ""}'
        user = f"\tuser: {phone}" if phone else ""
        print('[{name:<{len}}\tcode:{code:<{c_len}}\tmsg:{msg}\t{user:<{u_len}}'.format(
            name=label + ']',
            len=16 - len(label.encode('GBK')) + len(label),
            code=code,
            c_len=10 - len(str(code).encode('GBK')) + len(str(code)),
            msg=msg,
            user=user,
            u_len=16 - len(user.encode('GBK')) + len(user),
        ))
...
class Excel:
        ...
        try:
            ...
            for sheet in xlsx:
                ...
                for v in xlsx[sheet]:
                    ...
                    data.append({
                        'name': 'YB_COOKIE',
                        'value': v[2] if len(v) >= 3 else '',
                        'remarks': f"{v[0]}|{v[1]}" if len(v) >= 2 else '',
                        'account': v[0],
                        'password': v[1],
                        'sheet': sheet
                    })
    ...
    def save_data(self, data):
        status = ['正常', '登录失败']
        try:
            if self.is_already_opened_in_write():
                return {'code': -1, 'msg': '请先关闭Excel文件'}

            xlsx = pe.get_data(self.path)
            temp = [['账号', '密码', 'Cookie', '状态']]
            for v in data:
                temp.append([f"{v['account']}", v['password'], v['value'], status[v['status']]])
            for sheet in xlsx:
                xlsx[sheet] = temp
            pe.save_data(self.path, xlsx)
            return {'code': 1, 'msg': 'excel保存成功'}
        except Exception as ex:
            return {'code': -2, 'msg': '写入excel失败' + str(ex)}

    def is_already_opened_in_write(self):
